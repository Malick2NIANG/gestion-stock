<nav class="navbar navbar-expand-lg navbar-light bg-light shadow-sm animate__animated animate__fadeInDown" style="position: relative; z-index: 1050;">
    <div class="container-fluid">
        ...
        <!-- Logo + Titre -->
        <a class="navbar-brand d-flex align-items-center" href="{{ route('responsable.dashboard') }}">
            <x-application-logo class="me-2" style="height: 30px;" />
            <span class="fw-bold text-primary fs-5">Gestion Stock</span>
        </a>

        <!-- Bouton responsive -->
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarResponsable">
            <span class="navbar-toggler-icon"></span>
        </button>

        <!-- Droite : utilisateur -->
        <div class="collapse navbar-collapse" id="navbarResponsable">
    ...
    <div id="sidebarMenu" class="position-fixed top-0 start-0 vh-100 bg-white shadow-lg border-end p-4 animate__animated animate__fadeInLeft"
        ...
        <ul class="nav flex-column fw-semibold">
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('responsable.dashboard') }}">
                    <i class="bi bi-house-door me-2 text-primary"></i>Accueil
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('responsable.dashboard.rapport') }}">
                    <i class="bi bi-bar-chart-line me-2 text-dark"></i>Dashboard
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('responsable.utilisateurs') }}">
                    <i class="bi bi-people-fill me-2 text-info"></i>Gestion des utilisateurs
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('responsable.ventes') }}">
                    <i class="bi bi-bar-chart-fill me-2 text-warning"></i>Historique des ventes
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('responsable.stock') }}">
                    <i class="bi bi-exclamation-triangle-fill me-2 text-danger"></i>Stock & alertes
                </a>
            </li>
<li class="nav-item mb-2">
    <a class="nav-link text-dark" href="{{ route('responsable.rapport') }}">
        <i class="bi bi-clipboard-data me-2 text-success"></i> Inventaire & Bilan
    </a>
</li>
            <li class="nav-item mb-2">
                <a class="nav-link text-dark" href="{{ route('profile.password.edit') }}">
                    <i class="bi bi-key-fill me-2 text-success"></i>Modifier mon mot de passe
                </a>
            </li>
        </ul>
